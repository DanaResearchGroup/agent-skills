...
SLUG="${SLUG:-unknown}"; BRANCH="${BRANCH:-unknown}"; _BRANCH="$BRANCH"
_SESSION_ID="$$-$(date +%s)"
# Settings the old gstack preamble resolved from a config file and a
# fleet of helper binaries. There is one user, one host and no team here, so the
# resolved values were always these. They are inlined as constants rather than
# re-derived on every invocation.
_TEL=off; _PROACTIVE=true; _EXPLAIN_LEVEL=default
REPO_MODE=solo; SESSION_KIND=interactive; _SESSION_KIND=interactive
MODEL_OVERLAY=claude; VENDORED_GSTACK=no
_CHECKPOINT_MODE=explicit; _CHECKPOINT_PUSH=false
GSTACK_PLAN_MODE="${CLAUDE_PLAN_FILE:+active}"; GSTACK_PLAN_MODE="${GSTACK_PLAN_MODE:-inactive}"
echo "SLUG: $SLUG  BRANCH: $BRANCH  PLAN_MODE: $GSTACK_PLAN_MODE"
```
...
[ -z "$P" ] && P="$HOME/.claude/skills/make-pdf/dist/pdf"
if [ -x "$P" ]; then
  echo "MAKE_PDF_READY: $P"
  alias _p_="$P"   # shellcheck alias helper (not exported)
  export P   # available as $P in subsequent blocks within the same skill invocation
else
  echo "MAKE_PDF_NOT_AVAILABLE (the prebuilt binary is missing; see the note in this skill about its build)"
fi
```

If `MAKE_PDF_NOT_AVAILABLE` is printed: tell the user the binary is not
built. The binary ships prebuilt at make-pdf/dist/pdf and its build tooling
lived in gstack, so it cannot currently be rebuilt from this repo.
...
In plan mode, allowed because they inform the plan: `$B`, `$D`, `codex exec`/`codex review`, writes to `~/.skills/`, writes to the plan file, and `open` for generated artifacts.
...
```bash
~/.claude/skills/bin/skill-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
