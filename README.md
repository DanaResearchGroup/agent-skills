...
| **Funding** | `auto-proposals` — for each open funding call in the proposals archive, propose candidate research topics grounded in the group's own track record, then (on approval) write an outline and a full draft; every write goes through a chokepoint that cannot overwrite or delete anything in an un-versioned Dropbox archive |
